package com.github.ug_dbg.json;
...
public class JSONObject {
	...
	protected String defaultIndentChar = " ";
	...
	public String toString() {
		try {
			return this.toString(0, this.defaultIndentChar);
		} catch (Exception e) {
			return null;
		}
	}
	 ...
	 * @param indentChars  the indentation characters (e.g. " ", "\t")
	 * @return a printable, displayable, portable, transmittable representation
	 * of the object, beginning with <code>{</code>&nbsp;<small>(left
	 * brace)</small> and ending with <code>}</code>&nbsp;<small>(right
	 * brace)</small>.
	 ...
	 */
	public String toString(int indentFactor, String indentChars) throws JSONException {
		StringWriter w = new StringWriter();
		synchronized (w.getBuffer()) {
			return this.write(w, indentFactor, 0, indentChars).toString();
		}
	}
	...
	public Writer write(Writer writer) throws JSONException {
		return this.write(writer, 0, 0, this.defaultIndentChar);
	}
	...
	static Writer writeValue(
		...
		if (value == null) {
			...
			((JSONObject) value).write(writer, indentFactor, indent, indentChars);
		} else if (value instanceof JSONArray) {
			((JSONArray) value).write(writer, indentFactor, indent, indentChars);
		} else if (value instanceof Map) {
			Map<?, ?> map = (Map<?, ?>) value;
			new JSONObject(map).write(writer, indentFactor, indent, indentChars);
		} else if (value instanceof Collection) {
			Collection<?> coll = (Collection<?>) value;
			new JSONArray(coll).write(writer, indentFactor, indent, indentChars);
		} else if (value.getClass().isArray()) {
			new JSONArray(value).write(writer, indentFactor, indent, indentChars);
		} else {
	...
	static void indent(Writer writer, int indent, String indentChars) throws IOException {
	 ...
	 * @param indentChars  The indentation characters
	 * @return The writer.
	 */
	public Writer write(Writer writer, int indentFactor, int indent, String indentChars)
		...
		try {
			...
			if (length == 1) {
				...
				try {
					writeValue(writer, entry.getValue(), indentFactor, indent, indentChars);
				} catch (Exception e) {
				...
				for (final Entry<String, ?> entry : this.entrySet()) {
					...
					indent(writer, newindent, indentChars);
					final String key = entry.getKey();
					writer.write(quote(key));
					writer.write(':');
					if (indentFactor > 0) {
					...
					try {
						writeValue(writer, entry.getValue(), indentFactor, newindent, indentChars);
					} catch (Exception e) {
						throw new JSONException("Unable to write JSONObject value for key: " + key, e);
					}
					commanate = true;
				...
				indent(writer, indent, indentChars);
			}
			writer.write('}');
			return writer;
